...
User::User(const std::string& username, const std::string& password, const std::string& userType)
    : userId(0), username(username), userType(userType) {
...
bool UserDAO::createUser(const User& user) {
    ...
    query << "INSERT INTO users (username, password, user_type, email, phone_number) VALUES ("
          ...
          << "'" << conn->escapeString(user.getUserType()) << "', "
...
bool UserDAO::updateUser(const User& user) {
    ...
    query << "UPDATE users SET "
          ...
          << "user_type = '" << conn->escapeString(user.getUserType()) << "', "
...
bool UserDAO::deactivateUser(int userId) {
    ...
    std::stringstream query;
    query << "UPDATE users SET is_active = FALSE WHERE user_id = " << userId;
...
bool UserDAO::changePassword(int userId, const std::string& oldPassword, const std::string& newPassword) {
    ...
    std::stringstream query;
    query << "UPDATE users SET password_hash = '" << conn->escapeString(hashedNewPassword) 
...
std::vector<std::unique_ptr<User>> UserDAO::searchUsers(const std::string& searchTerm) {
    ...
    std::stringstream query;
    query << "SELECT user_id, username, email, password_hash, first_name, last_name, phone, is_active, created_at, updated_at "
          << "FROM users WHERE username LIKE '%" << conn->escapeString(searchTerm) << "%' "
          << "OR email LIKE '%" << conn->escapeString(searchTerm) << "%' "
          << "OR first_name LIKE '%" << conn->escapeString(searchTerm) << "%' "
          << "OR last_name LIKE '%" << conn->escapeString(searchTerm) << "%' "
          << "ORDER BY username";
...
User* UserDAO::mapRowToUser(MYSQL_ROW row, unsigned long* lengths) {
    ...
    if (row[2]) user->setEmail(std::string(row[2], lengths[2]));
    if (row[3]) user->setPasswordHash(std::string(row[3], lengths[3]));
    if (row[4]) user->setFirstName(std::string(row[4], lengths[4]));
    if (row[5]) user->setLastName(std::string(row[5], lengths[5]));
    if (row[6]) user->setPhone(std::string(row[6], lengths[6]));
    if (row[7]) user->setIsActive(std::string(row[7]) == "1");
    if (row[8]) user->setCreatedAt(std::string(row[8], lengths[8]));
    if (row[9]) user->setUpdatedAt(std::string(row[9], lengths[9]));
