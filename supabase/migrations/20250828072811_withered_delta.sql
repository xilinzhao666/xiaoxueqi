...
SELECT '=== 错误处理测试 ===' AS section;

-- 1. 外键约束测试
SELECT '1. 外键约束测试' AS test_name;

-- 尝试插入不存在用户ID的医生（应该失败）
-- INSERT INTO doctors (user_id, name, department, working_hours) 
-- VALUES (9999, 'Test Doctor', 'Test Dept', '9-5');

-- 2. 唯一约束测试
SELECT '2. 唯一约束测试' AS test_name;

-- 尝试插入重复用户名（应该失败）
-- INSERT INTO users (username, password, user_type) 
-- VALUES ('dr_zhang', 'test', 'Doctor');

-- 尝试插入重复身份证号（应该失败）
-- INSERT INTO patients (user_id, name, gender, birth_date, id_number) 
-- VALUES (7, 'Test Patient', 'Male', '1990-01-01', '110101198503150001');
...
-- 删除测试创建的数据
DELETE FROM medications WHERE prescription_id = @test_prescription_id;
DELETE FROM prescriptions WHERE prescription_id = @test_prescription_id;
DELETE FROM cases WHERE case_id = @test_case_id;
DELETE FROM appointments WHERE appointment_id = @test_appointment_id;
DELETE FROM hospitalization WHERE patient_id = @new_patient_id;
DELETE FROM patients WHERE patient_id = @new_patient_id;
DELETE FROM doctors WHERE user_id = @new_doctor_user_id;
DELETE FROM users WHERE user_id IN (@new_doctor_user_id, @new_patient_user_id);
